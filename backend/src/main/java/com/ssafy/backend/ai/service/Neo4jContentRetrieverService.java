...
public class Neo4jContentRetrieverService {
    ...
    public List<Integer> retrieveTrackByMemberIdAndTagId(int memberId, int tagId, int trackCount) {
        Query query = new Query("Return " + trackCount + " recommended tracks from among the songs with tagId " + tagId + " for the member with id " + memberId);
